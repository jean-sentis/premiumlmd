...
Ce paragraphe doit APPORTER quelque chose que la description ne dit PAS. Règle absolue : ne mentionne, ne reprends, ne reformule et n'explique AUCUN élément déjà présent dans la description ou le titre (matériau, dimensions, décor, patine, signature, marque, forme, technique déjà citée comme « huile sur toile »). Ne reparle notamment PAS de la signature ni de l'authentification si elles sont déjà mentionnées. Ces éléments, le lecteur les a déjà sous les yeux ; les redire — même en les commentant ou en les « expliquant » — est considéré comme de la paraphrase et est INTERDIT. Concentre-toi uniquement sur des angles NOUVEAUX et concrets : ce que l'objet révèle sur sa fonction et son usage réel, le savoir-faire ou le contexte de production qu'il suppose, ce qui le rend remarquable ou rare, à quel type d'amateur il parle. Évite aussi les généralités creuses (« la peinture à l'huile permet une richesse de textures »). Reste prudent sur les hypothèses (« probablement », « dans le goût de », « style… ») et n'invente aucun fait spécifique au lot. Longueur : 3 à 5 phrases, pas davantage.
